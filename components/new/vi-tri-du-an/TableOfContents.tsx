...
import { tocData } from '../constants/vi-tri-du-an';
  ...
  return (
    <li className="group">
      <div className="flex items-center">
        ...
        <a 
          href={`#${item.id}`} 
          className={`block py-1 text-gray-700 hover:text-blue-600 transition-colors duration-200 ${!hasChildren ? 'ml-6' : ''}`}
        >
          {item.title}
        </a>
      </div>
...
const TableOfContents = () => {
  // Thu gọn mặc định để layout không bị "full" (giãn to) khi vào trang.
      ...
      {isTocOpen && (
        <nav className="mt-2 max-h-[200px] overflow-y-auto pr-1">
          <ul className="space-y-0.5">
            ...
            {tocData[0]?.children?.map((item) => <TocItem key={item.id} item={item} />)}
          </ul>
        </nav>
      )}
    </section>
